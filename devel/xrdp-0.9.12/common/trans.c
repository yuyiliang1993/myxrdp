...
enum SessionType mytrans_get_session_type(MyTransInfo_t*self,const char *session_name){
	if(self==NULL||session_name == NULL)
		return SES_ERROR;
	snprintf(self->fifoname1,sizeof(self->fifoname1),\
		"%s/%s",SESSIONPATH,session_name);
	snprintf(self->fifoname2,sizeof(self->fifoname2),\
		"%s/%s",SESSIONPATH,session_name);
	if(!isFileExisted(self->fifoname1)){
		self->session_type = SES_MASTER;
	}
	else if(!isFileExisted(self->fifoname1)){
		self->session_type = SES_SLAVE;
	}
	else{
		self->session_type = SES_OTHER;
	}
	return self->session_type;
}

int mytrans_open_fifo_rw(const char *filename){
	if(create_fifo(filename) != 0){
		return (-1);
	}
	return open_fifo(filename,0666);
}


int mytrans_init_fifo(MyTransInfo_t*self,const char *session_name){
	if(self == NULL || session_name==NULL)
		return (-1);
	int type = self->session_type;
	if(type == SES_MASTER || type == SES_SLAVE){
		self->fifoFd[0] = mytrans_open_fifo_rw(self->fifoname1);
		self->fifoFd[1] = mytrans_open_fifo_rw(self->fifoname2);
...
void mytrans_init(MyTransInfo_t *p){
	if(p == NULL)
		return ;
	p->fifoFd[0] = p->fifoFd[1] = -1;
	p->session_type = 0;
	p->fifoname1[0]=p->fifoname2[0]='\0';
}

void mytrans_delete(MyTransInfo_t *p){
	if(p == NULL)
		return ;
	if(p->fifoFd[0] >=0 )
		closeFd(p->fifoFd[0]);
	if(p->fifoFd[1] >= 0)
		closeFd(p->fifoFd[1]);
	p->session_type = 0;
	if(strlen(p->fifoname1) >0)
		deleteFile(p->fifoname1);
	if(strlen(p->fifoname2) >0)
		deleteFile(p->fifoname2);
}
