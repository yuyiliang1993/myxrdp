...
int myxrdp_set_connection_rdp(	 struct xrdp_mm* self)
{
	if(self == NULL)
		return -1;
	//模块动态库
...
int myxrdp_conn_manager(struct xrdp_wm *self){
	...
	session_arg_t arg;
	MyTransInfo_t *pmytrans = &self->session->trans->mytrans;
	const char *sessionId = self->session->client_info->username;
	log_message(LOG_LEVEL_DEBUG,"sessionId:%s",sessionId);
	int type = mytrans_get_session_type(pmytrans,sessionId);
	if(type == SES_MASTER){
		log_message(LOG_LEVEL_DEBUG,"I am master session,pid-%d",getpid());
		if(mytrans_init_fifo(pmytrans,sessionId) < 0){
			log_message(LOG_LEVEL_ERROR,"mytrans_init_fifo:%s",strerror(errno));
			return 1;
		}
#if 1
		//wait slave session write connection argments
		if(is_fd_can_read(pmytrans->fifoFd[1],10000000)==0){
			log_message(LOG_LEVEL_ERROR,\
				"wait slave write connection argments timeouts:10000000us");
				return 1;
			}
			else {
				if(myRead(pmytrans->fifoFd[1], &arg,sizeof(arg))<0){
...
#endif		 
		//代填用户信息
		myxrdp_set_connection_rdp(self->mm);
		...
		if(myWrite(pmytrans->fifoFd[1],&arg,sizeof(arg))<=0){
		...
		log_message(LOG_LEVEL_DEBUG,"I am other session,pid-%d",getpid());
	}
	return 1;
}
...
xrdp_wm_check_wait_objs(struct xrdp_wm *self)
{
    ...
    if (g_is_wait_obj_set(self->login_mode_event))
    {
        ...
        rv1=xrdp_wm_login_mode_changed(self);
    }
